...
Common problems and solutions for TiRoute.
...
| Dashboard opens on wrong port | Set `PORT=1807` and `NEXT_PUBLIC_BASE_URL=http://localhost:1807` |
| No request logs under `logs/` | Set `ENABLE_REQUEST_LOGS=true`                                     |
| EACCES: permission denied     | Set `DATA_DIR=/path/to/writable/dir` to override `~/.tiroute`    |
...
TiRoute auto-refreshes tokens. If issues persist:
...
1. Verify `BASE_URL` points to your running instance (e.g., `http://localhost:1807`)
2. Verify `CLOUD_URL` points to your cloud endpoint (e.g., `https://tiroute.dev`)
...
1. Check runtime fields: `curl http://localhost:1807/api/cli-tools/runtime/codex | jq`
...
```bash
curl -s http://localhost:1807/api/cli-tools/codex-settings | jq '{installed,runnable,commandPath,runtimeMode,reason}'
curl -s http://localhost:1807/api/cli-tools/claude-settings | jq '{installed,runnable,commandPath,runtimeMode,reason}'
curl -s http://localhost:1807/api/cli-tools/openclaw-settings | jq '{installed,runnable,commandPath,runtimeMode,reason}'
```
...
```bash
# Health dashboard
http://localhost:1807/dashboard/health

# API health check
curl http://localhost:1807/api/monitoring/health
```
...
When many concurrent requests hit a rate-limited provider, TiRoute uses mutex + auto rate-limiting to serialize requests and prevent cascading failures. This is automatic for API key providers.

---

## Optional RAG / LLM failure taxonomy (16 problems)

Some TiRoute users place the gateway in front of RAG or agent stacks. In those setups it is common to see a strange pattern: TiRoute looks healthy (providers up, routing profiles ok, no rate limit alerts) but the final answer is still wrong.
...
1. When you investigate a bad response, capture:
   ...
   - route or provider combo in TiRoute
   - any RAG context used downstream (retrieved documents, tool calls, etc)
2. Map the incident to one or two WFGY ProblemMap numbers (`No.1` … `No.16`).
3. Store the number in your own dashboard, runbook, or incident tracker next to the TiRoute logs.
...
You can ignore this section if you do not run RAG or agent pipelines behind TiRoute.

---

## Still Stuck?

- **GitHub Issues**: [github.com/diegosouzapw/TiRoute/issues](https://github.com/diegosouzapw/TiRoute/issues)
