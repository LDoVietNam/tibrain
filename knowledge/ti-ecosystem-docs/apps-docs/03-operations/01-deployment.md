# TiRoute — Guia de Deploy em VM com Cloudflare

Guia completo para instalar e configurar o TiRoute em uma VM (VPS) com domínio gerenciado via Cloudflare.
...
# === Cloud Sync (opcional) ===
# CLOUD_URL=https://cloud.tiroute.online
# NEXT_PUBLIC_CLOUD_URL=https://cloud.tiroute.online
EOF
```
...
```bash
docker pull diegosouzapw/tiroute:latest

docker run -d \
  --name tiroute \
  --restart unless-stopped \
  --env-file /opt/tiroute/.env \
  -p 1807:1807 \
  -v tiroute-data:/app/data \
  diegosouzapw/tiroute:latest
```

### 2.4 Verificar se está rodando

```bash
docker ps | grep tiroute
docker logs tiroute --tail 20
```

Deve exibir: `[DB] SQLite database ready` e `listening on port 1807`.
...
```bash
cat > /etc/nginx/sites-available/tiroute << 'NGINX'
...
server {
    ...
    location / {
        proxy_pass http://127.0.0.1:1807;
...
# Ativar TiRoute
ln -sf /etc/nginx/sites-available/tiroute /etc/nginx/sites-enabled/tiroute
...
```bash
docker pull diegosouzapw/tiroute:latest
docker stop tiroute && docker rm tiroute
docker run -d --name tiroute --restart unless-stopped \
  --env-file /opt/tiroute/.env \
  -p 1807:1807 \
  -v tiroute-data:/app/data \
  diegosouzapw/tiroute:latest
...
```bash
docker logs -f tiroute          # Stream em tempo real
docker logs tiroute --tail 50   # Últimas 50 linhas
```
...
docker cp tiroute:/app/data ./backup-$(date +%F)

# Ou comprimir todo o volume
docker run --rm -v tiroute-data:/data -v $(pwd):/backup \
  alpine tar czf /backup/tiroute-data-$(date +%F).tar.gz /data
```
...
docker stop tiroute
docker run --rm -v tiroute-data:/data -v $(pwd):/backup \
  alpine sh -c "rm -rf /data/* && tar xzf /backup/tiroute-data-YYYY-MM-DD.tar.gz -C /"
docker start tiroute
```
...
```bash
# Impedir acesso externo direto à porta 1807
iptables -I DOCKER-USER -p tcp --dport 1807 -j DROP
iptables -I DOCKER-USER -i lo -p tcp --dport 1807 -j ACCEPT
...
Ver documentação completa em [tirouteCloud/README.md](../tirouteCloud/README.md).
...
| 1807 | TiRoute   | Somente localhost (via nginx) |
