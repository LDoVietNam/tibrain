...
All notable changes to TiRoute are documented in this file.
...
- **Notion Cookie Provider** — Support Notion AI via browser cookie (notion-2api style)
  - Auto-parse 3 formats: raw cookie string, JSON array, or simple JSON
  - Endpoint: `POST /api/settings/notion-cookie`
  - Models: claude-sonnet-4-20250514, claude-opus-4-5, gpt-4, gemini-1.5-pro
  - Auto-extract space_id from `p_sync_session` cookie

- **Updated Documentation**
  - README.md rewritten for Go implementation (port 1807)
  - Optimization guide updated (removed Node.js references)
  - Contributing guide updated for Go workflow

### 🐛 Fixed

- **Cookie Parsing** — Fixed `handleNotionCookie` to properly handle all 3 input formats (raw string, JSON array, simple JSON)
- **Provider Registration** — Added Notion Cookie to `bootstrap.go` provider registry
- **Build Fix** — Fixed Go build issue: Use `Z:\09_TOOLS\go\bin\go.exe` directly (GOROOT was misconfigured in Windows registry)

### 📝 Notes

- Config file: `data/notion-cookie.json` (auto-created on save)
- Requires restart after cookie update
- Port unified: **1807** across all docs
...
- **WFGY 16-Problem RAG Failure Map** — Added troubleshooting reference for RAG/LLM failure taxonomy in `docs/TROUBLESHOOTING.md` ([PR #164](https://github.com/diegosouzapw/TiRoute/pull/164) by @onestardao)

### 🐛 Fixed

- **Gemini Imported Models Return 404** — Strip `models/` prefix from Gemini model IDs during import to prevent doubled paths ([#163](https://github.com/diegosouzapw/TiRoute/issues/163))
- **Pino File Transport Fails in Next.js Production** — Log actual error + add sync `pino.destination()` fallback ([#165](https://github.com/diegosouzapw/TiRoute/issues/165))
- **Windows Electron CI Build** — Added `shell: bash` to Collect installers step for Windows runners ([PR #168](https://github.com/diegosouzapw/TiRoute/pull/168) by @benzntech)
...
- **16 Pain Points Documentation** — New collapsible section "🎯 What TiRoute Solves — 16 Real Pain Points" added to the main README and all 29 language-specific READMEs. Each pain point uses `<details>/<summary>` tags for clean, expandable content
- **Configurable User-Agent per Provider** — User-Agent strings for OAuth providers (Claude, Codex, GitHub, Antigravity, Kiro, iFlow, Qwen, Cursor, Gemini CLI) are now configurable via environment variables. Format: `{PROVIDER_ID}_USER_AGENT=custom-value` ([#155](https://github.com/diegosouzapw/TiRoute/issues/155))

### 🐛 Fixed

- **Hardcoded `$HOME` Path in Standalone/Bun Builds** — 5 files (`backupService.ts`, `mitm/manager.ts`, `mitm/server.ts`, `mitm/cert/generate.ts`, `codex-profiles/route.ts`) were bypassing the centralized `dataPaths.ts` and using `os.homedir()` directly. This caused paths to bake the build machine's `$HOME` into standalone/bun builds, producing `EACCES: permission denied` errors on other machines. All files now use `resolveDataDir()` from `dataPaths.ts`, respecting `DATA_DIR` env var and XDG conventions ([#156](https://github.com/diegosouzapw/TiRoute/issues/156))
...
- **Proxy Port Preservation** — `new URL()` silently strips default ports (80/443); proxy connections now extract the port from the raw URL string before parsing, preventing connection timeouts ([PR #161](https://github.com/diegosouzapw/TiRoute/pull/161))
- **Proxy Credential Encoding** — URL-encode special characters in proxy username/password; decode during legacy migration ([PR #161](https://github.com/diegosouzapw/TiRoute/pull/161))
- **HTTPS Proxy Default Port** — Changed from 8080 to 443 in frontend and migration logic ([PR #161](https://github.com/diegosouzapw/TiRoute/pull/161))
- **Proxy Dispatcher Cache** — Invalidate cached dispatchers when proxy config is updated or deleted ([PR #161](https://github.com/diegosouzapw/TiRoute/pull/161))
- **Proxy Logger SQLite Type** — Cast `proxyPort` to `Number` for INTEGER column ([PR #161](https://github.com/diegosouzapw/TiRoute/pull/161))
- **CopilotToolCard URL** — Use `baseUrl` prop directly instead of redundant `window.location.origin`; filter to chat models only (`!m.type && !m.parent`) ([PR #160](https://github.com/diegosouzapw/TiRoute/pull/160))
...
- **Electron Release Workflow** — Refactored CI to trigger on git tags (`v*`) + manual dispatch, with version validation, artifact upload/download pattern across 3 platforms, and a single release job. Only installer files (`.dmg`, `.exe`, `.AppImage`) are uploaded — no more 5K+ unpacked files ([PR #159](https://github.com/diegosouzapw/TiRoute/pull/159))
- **Windows Portable Exe** — Added standalone portable `.exe` build alongside the NSIS installer ([PR #159](https://github.com/diegosouzapw/TiRoute/pull/159))
- **Source Code Archives** — Releases now include `TiRoute-vX.Y.Z.source.tar.gz` and `.zip` via `git archive` ([PR #159](https://github.com/diegosouzapw/TiRoute/pull/159))
- **Installation Docs** — Added platform-specific installation instructions with macOS Gatekeeper workaround ([PR #159](https://github.com/diegosouzapw/TiRoute/pull/159))
...
- **GitHub Copilot Configuration Generator** — New tool on the CLI Tools dashboard page. Select models and generate the `chatLanguageModels.json` config block for VS Code GitHub Copilot using the Azure vendor pattern. Features: bulk model selection from `/v1/models` (includes combos/custom), search/filter, configurable tokens/tool-calling/vision, one-click copy, persistent selection via localStorage. Version compatibility warning for VS Code ≥ 1.109 / Copilot Chat ≥ v0.37 ([#142](https://github.com/diegosouzapw/TiRoute/issues/142))
...
- **Official app icons** — Added proper platform-specific icons derived from the TiRoute SVG logo: `.icns` (macOS), `.ico` (Windows), `.png` (Linux), and `tray-icon.png` (32×32) — via PR [#154](https://github.com/diegosouzapw/TiRoute/pull/154)
...
- Removed dead `tiroute://` protocol config — no handler existed (#14)
...
- **Provider labels in Combos** — Combo cards now show user-defined provider names instead of long UUID identifiers, making complex multi-provider combos easier to read ([#121](https://github.com/diegosouzapw/TiRoute/issues/121))
...
- **Cross-platform npm install** — Added `postinstall` script to auto-rebuild `better-sqlite3` for the user's OS/architecture. Previously, the npm package shipped Linux x64 binaries that failed on Windows and macOS ([#129](https://github.com/diegosouzapw/TiRoute/issues/129))
...
>
> API and Dashboard can now run on separate ports for advanced deployment scenarios (reverse proxies, container networking, network isolation). Community contribution by [@npmSteven](https://github.com/npmSteven) — PR [#140](https://github.com/diegosouzapw/TiRoute/pull/140).

### ✨ New Features

- **Split-Port Runtime** — Serve dashboard and OpenAI-compatible API on different ports via `API_PORT` and `DASHBOARD_PORT` env vars. Opt-in; single-port mode unchanged ([#140](https://github.com/diegosouzapw/TiRoute/pull/140))
...
- Bump `actions/upload-artifact` from 4 to 7 ([#143](https://github.com/diegosouzapw/TiRoute/pull/143))
- Bump `actions/download-artifact` from 4 to 8 ([#144](https://github.com/diegosouzapw/TiRoute/pull/144))
...
- **EACCES on Restricted Home Directories** — Fixed crash when `~/.tiroute` directory cannot be created due to permission issues. Now gracefully warns and suggests using the `DATA_DIR` environment variable (#133)
...
- **Custom Provider Model Persistence** — Compatible provider models (manual or imported) are now saved to the `customModels` database so they appear in `/v1/models` listing for clients like Cursor, Cline, Antigravity, and Claude Code ([PR #122](https://github.com/diegosouzapw/TiRoute/pull/122) by [@nyatoru](https://github.com/nyatoru))
...
- **Dedicated API Key Manager** — New `/dashboard/api-manager` page for managing API keys, extracted from the Endpoint page. Includes create, delete, and permissions management with a clean table UI ([PR #118](https://github.com/diegosouzapw/TiRoute/pull/118) by [@nyatoru](https://github.com/nyatoru))
...
- **Connection Tests Now Use Proxy** — Provider connection tests (`Test Connection` button) now route through the configured proxy (key → combo → provider → global → direct), matching the behavior of real API calls. Previously, `fetch()` was called directly, bypassing the proxy entirely ([#119](https://github.com/diegosouzapw/TiRoute/issues/119))
- **Compatible Provider Display in Logs** — OpenAI/Anthropic compatible providers now show friendly labels (`OAI-COMPAT`, `ANT-COMPAT`) instead of raw UUID-based IDs in the request logger's provider column, dropdown, and quick filters ([#113](https://github.com/diegosouzapw/TiRoute/issues/113))
...
- **Hide Health Check Logs** — New toggle in Settings → Appearance to suppress verbose `[HealthCheck]` messages from the server console. Uses a 30-second cache to minimize database reads with request coalescing for concurrent calls ([PR #111](https://github.com/diegosouzapw/TiRoute/pull/111) by [@nyatoru](https://github.com/nyatoru))
- **Kilocode Custom Models Endpoint** — Added `modelsUrl` support in `RegistryEntry` for providers with non-standard model endpoints. Expanded Kilocode model list from 8 to 26 models including Qwen3, GPT-5, Claude 3 Haiku, Gemini 2.5, DeepSeek V3, Llama 4, and more ([PR #115](https://github.com/diegosouzapw/TiRoute/pull/115) by [@benzntech](https://github.com/benzntech))

### 🐛 Bug Fixes

- **iFlow 406 Error** — Created dedicated `IFlowExecutor` with HMAC-SHA256 signature support (`session-id`, `x-iflow-timestamp`, `x-iflow-signature` headers). The iFlow provider was previously using the default executor which lacked the required signature headers, causing 406 errors ([#114](https://github.com/diegosouzapw/TiRoute/issues/114))
- **Duplicate Models in Endpoint Lists** — Filtered out parent models (`!m.parent`) from all model categorization and count logic on the Endpoint page. Provider modal lists also exclude duplicates ([PR #112](https://github.com/diegosouzapw/TiRoute/pull/112) by [@nyatoru](https://github.com/nyatoru))
...
- **JWT Session Auth Fallback** — When `requireAuthForModels` is enabled, the `/v1/models` endpoint now accepts both API key (Bearer token) for external clients **and** the dashboard JWT session cookie (`auth_token`), allowing logged-in dashboard users to view models without needing an explicit API key ([PR #110](https://github.com/diegosouzapw/TiRoute/pull/110) by [@nyatoru](https://github.com/nyatoru))
...
- **API Key Creation** — Added deterministic fallback for `API_KEY_SECRET` to prevent `crypto.createHmac` crash when the environment variable is not configured. Keys created without the secret are insecure (warned at startup) but the application no longer crashes ([#108](https://github.com/diegosouzapw/TiRoute/issues/108))
- **Codex Code Review Quota** — Added parsing of the third rate limit window (`code_review_rate_limit`) from the ChatGPT usage API, supporting Plus/Pro/Team plan differences. The dashboard now displays all three quota bars: Session (5h), Weekly, and Code Review ([#106](https://github.com/diegosouzapw/TiRoute/issues/106))
...
- **OAuth Client Secret** — Omitted explicitly empty `client_secret` parameters to resolve token exchange connection rejection on remote servers missing environment variables for Antigravity, Gemini and iFlow ([#103](https://github.com/diegosouzapw/TiRoute/issues/103))
- **Codex Business Quotas** — Automatically fetches the appropriate ChatGPT workspace to unlock the 5-hour Business usage limits directly inside the Quota tab and mapped `BIZ` string variant perfectly ([#101](https://github.com/diegosouzapw/TiRoute/issues/101))
...
- **OAuth Accounts** — Extracted user email from the `id_token` using JWT decoding for Qwen and similar providers, allowing multiple accounts of the same provider to be authenticated simultaneously instead of triggering the fallback overwrite logic ([#99](https://github.com/diegosouzapw/TiRoute/issues/99))
...
- **Windows Server Startup** — Fixed `ERR_INVALID_FILE_URL_PATH` crash on Windows by safely wrapping `import.meta.url` resolution with a fallback to `process.cwd()` for globally installed npm packages ([#98](https://github.com/diegosouzapw/TiRoute/issues/98))
- **Combo buttons visibility** — Fixed layout overlap and tight spacing for the Quick Action buttons (Clone / Delete / Test) on the Combos page on narrower screens ([#95](https://github.com/diegosouzapw/TiRoute/issues/95))
...
- **`stream` defaults to `false`** — Aligns with the OpenAI specification which explicitly states `stream` defaults to `false`. Previously TiRoute defaulted to `true`, causing SSE data to be returned instead of a JSON object, breaking clients like Spacebot, OpenCode, and standard Python/Rust/Go OpenAI SDKs that don't explicitly set `stream: true` ([#89](https://github.com/diegosouzapw/TiRoute/issues/89))
- **Custom AI providers now appear in `/v1/models`** — OpenAI-compatible custom providers (e.g. FriendLI) whose provider ID wasn't in the built-in alias map were silently excluded from the models list even when active. Fixed by also checking the raw provider ID from the database against active connections ([#90](https://github.com/diegosouzapw/TiRoute/issues/90))
- **OAuth `redirect_uri_mismatch` — improved UX for remote deployments** — Google OAuth providers (Antigravity, Gemini CLI) now always use `localhost` as redirect URI matching the registered credentials. Remote-access users see a targeted amber warning with a link to the new setup guide. The token exchange error message explains the root cause and guides users to configure their own credentials ([#91](https://github.com/diegosouzapw/TiRoute/issues/91))
...
> ### 🎉 First Major Release — TiRoute 1.0
>
> TiRoute is an intelligent API gateway that unifies 20+ AI providers behind a single OpenAI-compatible endpoint. This release represents the culmination of the entire development effort — from initial prototype to production-ready platform.
...
- **Docker Hub** — `diegosouzapw/tiroute` with `latest` and versioned tags
- **Docker CI/CD** — GitHub Actions auto-build and push on release
- **npm CLI package** — `npx tiroute` with auto-launch
...
[1.6.3]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.6.3
[1.6.2]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.6.2
[1.6.1]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.6.1
[1.6.0]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.6.0
[1.5.0]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.5.0
[1.4.11]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.4.11
[1.4.10]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.4.10
[1.4.9]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.4.9
[1.4.8]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.4.8
[1.4.7]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.4.7
[1.4.6]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.4.6
[1.4.5]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.4.5
[1.4.4]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.4.4
[1.4.3]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.4.3
[1.4.2]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.4.2
[1.4.1]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.4.1
[1.4.0]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.4.0
[1.3.1]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.3.1
[1.3.0]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.3.0
[1.2.0]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.2.0
[1.1.1]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.1.1
[1.0.7]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.0.7
[1.0.6]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.0.6
[1.0.5]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.0.5
[1.0.4]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.0.4
[1.1.0]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.1.0
[1.0.1]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.0.1
[1.0.3]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.0.3
[1.0.2]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.0.2
[1.0.0]: https://github.com/diegosouzapw/TiRoute/releases/tag/v1.0.0
