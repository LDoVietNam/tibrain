...
TiRoute is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[TiRoute Local Process]
...
- `src/lib/localDb.ts`
- file: `${DATA_DIR}/db.json` (or `$XDG_CONFIG_HOME/tiroute/db.json` when set, else `~/.tiroute/db.json`)
...
- follows same base directory policy as `localDb` (`DATA_DIR`, then `XDG_CONFIG_HOME/tiroute` when set)
...
- main state: `${DATA_DIR}/db.json` (or `$XDG_CONFIG_HOME/tiroute/db.json` when set, else `~/.tiroute/db.json`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[TiRoute Runtime]
        Next[Next.js Server\nPORT=1807]
...
1. `usageDb` and `localDb` now share the same base directory policy (`DATA_DIR` -> `XDG_CONFIG_HOME/tiroute` -> `~/.tiroute`) with legacy file migration.
...
5. The `open-sse/` directory is published as the `@tiroute/open-sse` **npm workspace package**. Source code imports it via `@tiroute/open-sse/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
...
- Build Docker image: `docker build -t tiroute .`
- Start service and verify:
- `GET /api/settings`
- `GET /api/v1/models`
- CLI target base URL should be `http://<host>:1807/v1` when `PORT=1807`
