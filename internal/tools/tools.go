...
// ToolRegistrar is the interface required to register tools.
// Implementations should register a tool with the given fields.
...
func RegisterDefaultTools(hub ToolRegistrar) error {
	...
	for _, def := range definitions {
		...
		if err := hub.RegisterToolByFields(
			def.ID, def.Name, def.Description,
			string(parametersJSON), "router", def.Category,
			string(permissionsJSON), true,
		); err != nil {
